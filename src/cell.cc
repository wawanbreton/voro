...
void voronoicell_base::add_memory_vertices(vc_class& vc)
{
    ...
    mask = pmask;
    ppts = new double[i << 2];
    for (j = 0; j < (current_vertices << 2); j++)
        ppts[j] = pts[j];
    delete[] pts;
    pts = ppts;
    current_vertices = i;
...
void voronoicell_base::init_tetrahedron_base(
    double x0,
    double y0,
    double z0,
    double x1,
...
inline bool voronoicell_base::search_downward(unsigned int& lw, int& lp, int& ls, int& us, double& l, double& u)
{
    ...
    while (lw == 2)
    {
        ...
        if (us == nu[up] && definite_min(lp, us, l, u, lw))
            return false;
    }
    ls = ed[up][nu[up] + us];
    return true;
...
bool voronoicell_base::nplane(vc_class& vc, double x, double y, double z, double rsq, int p_id)
{
    ...
    if (uw == 2)
    {
        if (! search_downward(lw, lp, ls, us, l, u))
            return false;
        if (lw == 1)
        {
            up = lp;
    ...
    if (create_facet(vc, lp, ls, l, us, u, p_id))
        return false;
    int k = 0;
    int xtra = 0;
    while (xse + k < stackp3)
    {
        ...
        for (ls = 0; ls < nu[lp]; ls++)
        {
            ...
            else if (uw == 1)
            {
                ...
                if (create_facet(vc, -1, 0, 0, 0, u, p_id))
                    return false;
            }
            else
            {
...
{
    double ix = pts[4 * i], iy = pts[4 * i + 1], iz = pts[4 * i + 2], kx = pts[4 * k], ky = pts[4 * k + 1], kz = pts[4 * k + 2],
           mx = pts[4 * m], my = pts[4 * m + 1], mz = pts[4 * m + 2], ux = kx - ix, uy = ky - iy, uz = kz - iz, vx = mx - kx, vy = my - ky,
           vz = mz - kz, e1x = uz * vy - uy * vz, e1y = ux * vz - uz * vx, e1z = uy * vx - ux * vy, e2x, e2y, e2z,
    ...
    double e3x = e1z * e2y - e1y * e2z, e3y = e1x * e2z - e1z * e2x, e3z = e1y * e2x - e1x * e2y, x0 = e1x * ix + e1y * iy + e1z * iz;
    if (x0 < tol)
        return;

    double ir = e2x * ix + e2y * iy + e2z * iz, is = e3x * ix + e3y * iy + e3z * iz, kr = e2x * kx + e2y * ky + e2z * kz,
           ks = e3x * kx + e3y * ky + e3z * kz, mr = e2x * mx + e2y * my + e2z * mz, ms = e3x * mx + e3y * my + e3z * mz;
...
void voronoicell_base::minkowski_formula(double x0, double y0, double z0, double r, double& ar, double& vo)
{
    ...
    double xs = x0 * x0, ys = y0 * y0, zs = z0 * z0, res = xs + ys, rvs = res + zs, theta = atan(z0 / y0), rs = r * r, rc = rs * r, temp,
           voc, arc;
    if (r < x0)
    {
    ...
    else if (rs < rvs)
    {
        ...
        double temp2 = (rs * x0 - xs * x0 / 3.), x2s = rs * xs / res, y2s = rs * ys / res, temp3 = asin((x2s - y2s - xs) / (rs - xs)),
               temp4 = asin((zs * xs - ys * rvs) / (res * (ys + zs))), temp5 = sqrt(rs - res);
        voc = 0.5 * temp * temp2 + x0 * y0 / 6. * temp5 + r * rs / 6 * (temp3 - temp4);
        arc = x0 * r * temp - 0.5 * temp2 * y0 * r / ((rs - xs) * temp5) + x0 * y0 / 6. * r / temp5 + rs * 0.5 * temp3
            + rs * rs / 3. * 2 * xs * ys / (res * (rs - xs) * sqrt((rs - xs) * (rs - xs) - (x2s - y2s - xs) * (x2s - y2s - xs)))
            - rs * 0.5 * temp4;
    }
    else
    {
...
inline bool voronoicell_base::plane_intersects_track(double x, double y, double z, double rsq, double g)
{
    for (int tp = 0; tp < p; tp++)
        if (x * pts[tp << 2] + y * pts[(tp << 2) + 1] + z * pts[(tp << 2) + 2] > rsq)
            return true;
    return false;
    /*
        int ls,us,lp;
 ...
 * \param (x2,y2,z2) a position vector for the third vertex.
 * \param (x3,y3,z3) a position vector for the fourth vertex. */
void voronoicell_neighbor::init_tetrahedron(
    double x0,
    double y0,
    double z0,
    double x1,
