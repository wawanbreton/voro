...
{
    puts(
        "The \"-c\" option allows a string to be specified that will customize the output\n"
...
int main(int argc, char** argv)
{
    ...
    while (i < argc - 7)
    {
        ...
        else if (strcmp(argv[i], "-n") == 0)
        {
            ...
            {
                fputs(
                    "voro++: Computational grid specified with -n must be greater than one\n"
                    "in each direction\n",
                    stderr);
                wl.deallocate();
                return VOROPP_CMD_LINE_ERROR;
    ...
    if (verbose)
    {
        printf(
            ...
            ny,
            nz,
            bm == none ? "estimated from file" : (bm == length_scale ? "estimated using length scale" : "directly specified"),
            argv[i + 6],
            c_str,
